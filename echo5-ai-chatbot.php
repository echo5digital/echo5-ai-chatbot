...
function echo5_chatbot_ajax_send_message() {
	...
	if ( is_wp_error( $response_data ) ) {
		...
		// Assuming $response_data is an array like ['reply' => 'AI message']
...
function echo5_chatbot_activate() {
	...
	// It's a good practice to check if WP_DEBUG_LOG is enabled or use a more robust logging mechanism for production.
	if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
		error_log( 'Echo5 AI Chatbot activated successfully!' ); // phpcs:ignore WordPress.PHP.DevelopmentFunctions.error_log_error_log
	}
}
